...
function c100256014.initial_effect(c)
	...
	e1:SetTarget(c100256014.target)
	e1:SetOperation(c100256014.activate)
	c:RegisterEffect(e1)
end
function c100256014.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(aux.disfilter1,tp,0,LOCATION_MZONE,1,nil) end
	if e:IsHasType(EFFECT_TYPE_ACTIVATE) then
		Duel.SetChainLimit(c100256014.chainlimit)
	end
end
function c100256014.chainlimit(e,rp,tp)
	return not e:IsActiveType(TYPE_MONSTER)
end
function c100256014.activate(e,tp,eg,ep,ev,re,r,rp)
