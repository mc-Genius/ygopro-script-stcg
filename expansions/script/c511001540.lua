...
function c511001540.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCodeRep(c,511001539,2,false,true)
	c511001540.material_count=2
	--actlimit
	...
	e2:SetCountLimit(1)
	e2:SetCondition(c511001540.acidcon)
	e2:SetOperation(c511001540.acidop)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	...
	e4:SetDescription(aux.Stringid(511001540,1))
...
function c511001540.acidfilter(c,tp)
	return c:IsControler(tp) and c:IsFaceup() and c:GetCounter(0x1343)==0
end
function c511001540.acidcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c511001540.acidfilter,1,nil,1-tp)
end
function c511001540.acidop(e,tp,eg,ep,ev,re,r,rp)
	local g=eg:Filter(c511001540.acidfilter,nil,1-tp)
	local tc=g:GetFirst()
	while tc do
...
function c511001540.descon(e,tp,eg,ep,ev,re,r,rp,chk)
