--デバッグ－システム追加
function c9000.initial_effect(c)
	-- 相手フィールドにフィールド魔法を創造する
	local e1=Effect.CreateEffect(c)
	e1:SetProperty(EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_CANNOT_DISABLE)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e1:SetCode(EVENT_PREDRAW)
	e1:SetCountLimit(1,9000+EFFECT_COUNT_CODE_DUEL)
	e1:SetRange(0xf7)
	e1:SetOperation(c9000.op)
	c:RegisterEffect(e1)
	--カードを創造する
	local ge1=Effect.CreateEffect(c)
	ge1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	ge1:SetCode(EVENT_PREDRAW)
	ge1:SetOperation(c9000.sdop)
	ge1:SetCountLimit(1,9000)
	ge1:SetCondition(c9000.sdcon)
	...
	ge2:SetOperation(c9000.sdop2)
	ge2:SetCountLimit(1,9000)
	Duel.RegisterEffect(ge2,0)
end
function c9000.sdop2(e,tp,eg,ep,ev,re,r,rp)
	...
	local c=e:GetHandler():GetOwner()
	if Duel.SelectYesNo(c,aux.Stringid(9000,2)) then
...
function c9000.sdop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler():GetOwner()
	Duel.Remove(e:GetHandler(),LOCATION_GRAVE,POS_FACEUP)
	local c=e:GetHandler():GetOwner()
	if Duel.SelectYesNo(c,aux.Stringid(9000,0)) then
	local tc=Duel.AnnounceCard(c)
	Duel.SetTargetParam(tc)
...
function c9000.sdcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:GetOwner()==Duel.GetTurnPlayer() and Duel.GetTurnCount()~=1
end
function c9000.op(e,tp,eg,ep,ev,re,r,rp,chk,c)
	local c=e:GetHandler():GetOwner()
	Duel.Remove(e:GetHandler(),LOCATION_GRAVE,POS_FACEUP)
	if Duel.SelectYesNo(c,aux.Stringid(9000,1)) then
	local tc=Duel.GetFieldCard(tp,LOCATION_SZONE,5)
	local tc2=Duel.GetFieldCard(1-tp,LOCATION_SZONE,5)
	if tc==nil then
		-- Duel.SendtoDeck(e:GetHandler(),tp,tp,REASON_RULE,POS_FACEUP,true)
		if tc2==nil then
			c9000.announce_filter={TYPE_FIELD,OPCODE_ISTYPE}
			local code=Duel.AnnounceCard(tp,table.unpack(c9000.announce_filter))
...
function c9000.setlimit(e,c,tp)
	return c:IsType(TYPE_FIELD)
end
function c9000.actlimit(e,re,tp)
